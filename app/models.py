...
class Product(db.Model):
    ...
    )  # NEW: Expiry date for perishable products
    tenant_id = db.Column(db.Integer, db.ForeignKey("tenant.id"), nullable=True)
...
class ProductBundle(db.Model):
    ...
    bundle = db.relationship(
        "Product", foreign_keys=[bundle_id], backref="bundle_components"
    )
...
class InboundOrder(db.Model):
    ...
    supplier = db.Column(
        db.String(200)
    )  # Nombre del proveedor (alternativa a supplier_id)
...
class Lot(db.Model):
    ...
    order_id = db.Column(db.Integer, db.ForeignKey("inbound_order.id"), nullable=True)
    tenant_id = db.Column(db.Integer, db.ForeignKey("tenant.id"), nullable=True)
...
class Truck(db.Model):
    ...
    )  # available, on_route, maintenance
    tenant_id = db.Column(db.Integer, db.ForeignKey("tenant.id"), nullable=True)
...
class VehicleMaintenance(db.Model):
    ...
    maintenance_type = db.Column(
        db.String(50)
    )  # 'oil_change', 'preventive', 'tire_rotation', 'inspection', 'repair'
